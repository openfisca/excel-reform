...
class TestExcelReformBuilder:
    def test_get_suffixes(self, wb_file):
        builder = ExcelReformBuilder(baseline=None, path_or_file=wb_file)
    ...
    def test_build_reform(self, tax_benefit_system, wb_file):
        builder = ExcelReformBuilder(baseline=tax_benefit_system, path_or_file=wb_file)
    ...
    def test_get_parameters_shuffled_columns(self, wb):
        ...
        builder = ExcelReformBuilder(baseline=None, path_or_file=to_wb_file(wb))
        assert builder.get_parameters("_2025") == [
            ("parenting_allowance.amount", 100000, date(2025, 1, 1))
        ]
...
class TestExcelReformTemplateGenerator:
    def test_generate_parameter_data(self, tax_benefit_system):
        period = date(2025, 1, 1)
        generator = ExcelReformTemplateGenerator(tax_benefit_system, "benefits", period)
    ...
    def test_generate_parameter_data_with_scale(self, tax_benefit_system):
        root_name = "taxes"
        period = date(2025, 1, 1)
        generator = ExcelReformTemplateGenerator(tax_benefit_system, root_name, period)
...
class TestExcelReformIntegration:
    def test_build_reform_excel_and_generate_template(self, tax_benefit_system):
        ...
        io_bytes.seek(0)
        ## Here you could update the Excel file

        builder = ExcelReformBuilder(baseline=tax_benefit_system, path_or_file=io_bytes)
